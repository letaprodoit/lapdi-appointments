...
<div id="opt_{$obj_prefix}{$id}">
	...
	<div class="form-field{if !$capture_options_vs_qty} product-list-field{/if} clearfix" id="opt_{$obj_prefix}{$id}_{$po.option_id}">
		...
		<label for="option_{$obj_prefix}{$id}_{$po.option_id}" class="{if $po.required == "Y"}cm-required{/if} {if $po.regexp}cm-regexp{/if}">{$po.option_name}{if $po.description}{include file="common_templates/tooltip.tpl" tooltip=$po.description}{/if}:&nbsp;</label>
				...
				<ul id="option_{$obj_prefix}{$id}_{$po.option_id}_group">
					...
					{if !$po.disabled && !$disabled}
						{foreach from=$po.variants item="vr" name="vars"}
							...
							{$vr.variant_name}&nbsp;{if  $show_modifiers}{hook name="products:options_modifiers"}{if $vr.modifier|floatval}({include file="common_templates/modifier.tpl" mod_type=$vr.modifier_type mod_value=$vr.modifier display_sign=true}){/if}{/hook}{/if}</label></li>
						{/foreach}
					{/if}
				</ul>
		...
		{elseif $po.option_type == "C"} {*Checkbox*}
			...
			{else}
				...
				{if $show_modifiers}{hook name="products:options_modifiers"}{if $vr.modifier|floatval}({include file="common_templates/modifier.tpl" mod_type=$vr.modifier_type mod_value=$vr.modifier display_sign=true}){/if}{/hook}{/if}</label>
			{/if}
			{foreachelse}
		...
		{elseif $po.option_type == "D"} {*Date*}
			<div class="clearfix">
				{include file="common_templates/calendar.tpl" date_id="option_`$obj_prefix``$id`_`$po.option_id`" date_name="`$name`[`$id`][product_options][`$po.option_id`]" start_year="`$smarty.const.TIME`" date_val=$po.value|default:$smarty.const.TIME}
			</div>
