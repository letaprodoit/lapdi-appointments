...
<div class="cm-picker-product-options" id="opt_{$obj_prefix}{$id}">
    ...
    <div class="control-group{if !$capture_options_vs_qty} product-list-field{/if} clearfix" id="opt_{$obj_prefix}{$id}_{$po.option_id}">
        ...
        <label for="option_{$obj_prefix}{$id}_{$po.option_id}" class="{if $po.required == "Y"}cm-required{/if} {if $po.regexp}cm-regexp{/if}">{$po.option_name}{if $po.description|strip_tags|trim}{include file="common/tooltip.tpl" tooltip=$po.description}{/if}:&nbsp;</label>
