...
<tr {if $cp.product_options}class="no-border"{/if}>
	...
	{else}
		...
		<tr>
			...
			{/if}
			<span {if $cp.stored_price == "Y"}class="hidden"{/if} id="db_price_{$key}">{include file="common/price.tpl" value=$original_price}</span>
...
<div class="buttons-container center buttons-bg">
	{if $cart_products}
		<div class="float-left">
			...
			{include file="common/tools.tpl" prefix="main" hide_actions=true tools_list=$smarty.capture.tools_list display="inline" link_text=$lang.choose_action}
		</div>
	{/if}
...
{/if}
{include file="common/mainbox.tpl" title=$_title content=$smarty.capture.mainbox extra_tools=$smarty.capture.extra_tools}