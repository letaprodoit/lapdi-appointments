...
<tr {cycle values="class=\"table-row\", "}>
	...
	<td>
		...
		{$product.product|unescape}{include file="addons/tsp_appointments/overrides/views/products/components/select_product_options.tpl" id=$product.product_id product_options=$product.product_options name="product_data" show_aoc=$show_aoc additional_class=$additional_class}</span></td>
	{if $show_price}
	<td>&nbsp;{if !$product.price|floatval && $product.zero_price_action == "A"}<input class="input-text" id="product_price_{$product.product_id}" type="text" size="3" name="product_data[{$product.product_id}][price]" value="" />{else}{include file="common/price.tpl" value=$product.price}{/if}</td>
	{/if}
	{if !$hide_amount}
