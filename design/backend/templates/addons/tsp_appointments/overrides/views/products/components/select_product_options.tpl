...
<div id="opt_{$id}_{$po.option_id}" class="form-field{if $additional_class} {$additional_class}{/if}">
	...
	{elseif $po.option_type == "C"} {*Checkbox*}
		...
		{else}
			<div class="select-field">
				...
				<label>{if $settings.General.display_options_modifiers == "Y"}{if $vr.modifier|floatval}&nbsp;({include file="common/modifier.tpl" mod_type=$vr.modifier_type mod_value=$vr.modifier display_sign=true}){/if}{hook name="products:select_options"}{/hook}{/if}</label>
			</div>
		{/if}
		{/foreach}
	...
	{elseif $po.option_type == "D"} {*Date*}
		<div class="clear">
			{include file="common/calendar.tpl" date_id="option_`$id`_`$po.option_id`" date_name="`$name`[`$id`][product_options][`$po.option_id`]" date_val=$po.value|default:$smarty.const.TIME start_year="`$smarty.const.TIME`"}
		</div>
